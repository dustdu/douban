<template>
<div>
	<loading v-model:show="hotDate.loading.load" text="加载中..."></loading>
  	<div v-if="hotDate.loading.con" class="box" ref="listBox">
		<div :style="{height:boxHeight}" class="movieList">
        <div v-for="item in hotDate.movieList.subjects" class="row clearfix" @click="link(item.id,item.title)">
            <div class="img">
...
export default {
  ...
  destroyed(){
    this.$store.commit("hotLoding");
  },
  computed: {
    hotDate() {
      return this.$store.state.hotDate;
    },
    boxHeight() {
      return this.$store.getters.hotHeight;
    }
  },
  methods: {
    link(id, title) {
